...
class AnalyzersConfig 
{
    ...
    {
        if (array_key_exists('n_plus_one', $config)) {
            $this->_usedProperties['nPlusOne'] = true;
            $this->nPlusOne = new \Symfony\Config\DoctrineDoctor\Analyzers\NPlusOneConfig($config['n_plus_one']);
            unset($config['n_plus_one']);
        }

        if (array_key_exists('slow_query', $config)) {
            $this->_usedProperties['slowQuery'] = true;
            $this->slowQuery = new \Symfony\Config\DoctrineDoctor\Analyzers\SlowQueryConfig($config['slow_query']);
            unset($config['slow_query']);
        }

        if (array_key_exists('missing_index', $config)) {
            $this->_usedProperties['missingIndex'] = true;
            $this->missingIndex = new \Symfony\Config\DoctrineDoctor\Analyzers\MissingIndexConfig($config['missing_index']);
            unset($config['missing_index']);
        }

        if (array_key_exists('hydration', $config)) {
            $this->_usedProperties['hydration'] = true;
            $this->hydration = new \Symfony\Config\DoctrineDoctor\Analyzers\HydrationConfig($config['hydration']);
            unset($config['hydration']);
        }

        if (array_key_exists('eager_loading', $config)) {
            $this->_usedProperties['eagerLoading'] = true;
            $this->eagerLoading = new \Symfony\Config\DoctrineDoctor\Analyzers\EagerLoadingConfig($config['eager_loading']);
            unset($config['eager_loading']);
        }

        if (array_key_exists('find_all', $config)) {
            $this->_usedProperties['findAll'] = true;
            $this->findAll = new \Symfony\Config\DoctrineDoctor\Analyzers\FindAllConfig($config['find_all']);
            unset($config['find_all']);
        }

        if (array_key_exists('entity_manager_clear', $config)) {
            $this->_usedProperties['entityManagerClear'] = true;
            $this->entityManagerClear = new \Symfony\Config\DoctrineDoctor\Analyzers\EntityManagerClearConfig($config['entity_manager_clear']);
            unset($config['entity_manager_clear']);
        }

        if (array_key_exists('get_reference', $config)) {
            $this->_usedProperties['getReference'] = true;
            $this->getReference = new \Symfony\Config\DoctrineDoctor\Analyzers\GetReferenceConfig($config['get_reference']);
            unset($config['get_reference']);
        }

        if (array_key_exists('flush_in_loop', $config)) {
            $this->_usedProperties['flushInLoop'] = true;
            $this->flushInLoop = new \Symfony\Config\DoctrineDoctor\Analyzers\FlushInLoopConfig($config['flush_in_loop']);
            unset($config['flush_in_loop']);
        }

        if (array_key_exists('lazy_loading', $config)) {
            $this->_usedProperties['lazyLoading'] = true;
            $this->lazyLoading = new \Symfony\Config\DoctrineDoctor\Analyzers\LazyLoadingConfig($config['lazy_loading']);
            unset($config['lazy_loading']);
        }

        if (array_key_exists('dql_injection', $config)) {
            $this->_usedProperties['dqlInjection'] = true;
            $this->dqlInjection = new \Symfony\Config\DoctrineDoctor\Analyzers\DqlInjectionConfig($config['dql_injection']);
            unset($config['dql_injection']);
        }

        if (array_key_exists('bulk_operation', $config)) {
            $this->_usedProperties['bulkOperation'] = true;
            $this->bulkOperation = new \Symfony\Config\DoctrineDoctor\Analyzers\BulkOperationConfig($config['bulk_operation']);
            unset($config['bulk_operation']);
        }

        if (array_key_exists('strict_mode', $config)) {
            $this->_usedProperties['strictMode'] = true;
            $this->strictMode = new \Symfony\Config\DoctrineDoctor\Analyzers\StrictModeConfig($config['strict_mode']);
            unset($config['strict_mode']);
        }

        if (array_key_exists('charset', $config)) {
            $this->_usedProperties['charset'] = true;
            $this->charset = new \Symfony\Config\DoctrineDoctor\Analyzers\CharsetConfig($config['charset']);
            unset($config['charset']);
        }

        if (array_key_exists('inno_db_engine', $config)) {
            $this->_usedProperties['innoDbEngine'] = true;
            $this->innoDbEngine = new \Symfony\Config\DoctrineDoctor\Analyzers\InnoDbEngineConfig($config['inno_db_engine']);
            unset($config['inno_db_engine']);
        }

        if (array_key_exists('connection_pooling', $config)) {
            $this->_usedProperties['connectionPooling'] = true;
            $this->connectionPooling = new \Symfony\Config\DoctrineDoctor\Analyzers\ConnectionPoolingConfig($config['connection_pooling']);
            unset($config['connection_pooling']);
        }

        if (array_key_exists('collection_initialization', $config)) {
            $this->_usedProperties['collectionInitialization'] = true;
            $this->collectionInitialization = new \Symfony\Config\DoctrineDoctor\Analyzers\CollectionInitializationConfig($config['collection_initialization']);
            unset($config['collection_initialization']);
        }

        if (array_key_exists('cascade_configuration', $config)) {
            $this->_usedProperties['cascadeConfiguration'] = true;
            $this->cascadeConfiguration = new \Symfony\Config\DoctrineDoctor\Analyzers\CascadeConfigurationConfig($config['cascade_configuration']);
            unset($config['cascade_configuration']);
        }

        if (array_key_exists('sensitive_data_exposure', $config)) {
            $this->_usedProperties['sensitiveDataExposure'] = true;
            $this->sensitiveDataExposure = new \Symfony\Config\DoctrineDoctor\Analyzers\SensitiveDataExposureConfig($config['sensitive_data_exposure']);
            unset($config['sensitive_data_exposure']);
        }

        if (array_key_exists('insecure_random', $config)) {
            $this->_usedProperties['insecureRandom'] = true;
            $this->insecureRandom = new \Symfony\Config\DoctrineDoctor\Analyzers\InsecureRandomConfig($config['insecure_random']);
            unset($config['insecure_random']);
        }

        if (array_key_exists('sql_injection_raw_queries', $config)) {
            $this->_usedProperties['sqlInjectionRawQueries'] = true;
            $this->sqlInjectionRawQueries = new \Symfony\Config\DoctrineDoctor\Analyzers\SqlInjectionRawQueriesConfig($config['sql_injection_raw_queries']);
            unset($config['sql_injection_raw_queries']);
        }

        if (array_key_exists('foreign_key_mapping', $config)) {
            $this->_usedProperties['foreignKeyMapping'] = true;
            $this->foreignKeyMapping = new \Symfony\Config\DoctrineDoctor\Analyzers\ForeignKeyMappingConfig($config['foreign_key_mapping']);
            unset($config['foreign_key_mapping']);
        }

        if (array_key_exists('partial_object', $config)) {
            $this->_usedProperties['partialObject'] = true;
            $this->partialObject = new \Symfony\Config\DoctrineDoctor\Analyzers\PartialObjectConfig($config['partial_object']);
            unset($config['partial_object']);
        }

        if (array_key_exists('dto_hydration', $config)) {
            $this->_usedProperties['dtoHydration'] = true;
            $this->dtoHydration = new \Symfony\Config\DoctrineDoctor\Analyzers\DtoHydrationConfig($config['dto_hydration']);
            unset($config['dto_hydration']);
        }

        if (array_key_exists('cascade_all', $config)) {
            $this->_usedProperties['cascadeAll'] = true;
            $this->cascadeAll = new \Symfony\Config\DoctrineDoctor\Analyzers\CascadeAllConfig($config['cascade_all']);
            unset($config['cascade_all']);
        }

        if (array_key_exists('cascade_persist_independent', $config)) {
            $this->_usedProperties['cascadePersistIndependent'] = true;
            $this->cascadePersistIndependent = new \Symfony\Config\DoctrineDoctor\Analyzers\CascadePersistIndependentConfig($config['cascade_persist_independent']);
            unset($config['cascade_persist_independent']);
        }

        if (array_key_exists('missing_orphan_removal', $config)) {
            $this->_usedProperties['missingOrphanRemoval'] = true;
            $this->missingOrphanRemoval = new \Symfony\Config\DoctrineDoctor\Analyzers\MissingOrphanRemovalConfig($config['missing_orphan_removal']);
            unset($config['missing_orphan_removal']);
        }

        if (array_key_exists('cascade_remove_independent', $config)) {
            $this->_usedProperties['cascadeRemoveIndependent'] = true;
            $this->cascadeRemoveIndependent = new \Symfony\Config\DoctrineDoctor\Analyzers\CascadeRemoveIndependentConfig($config['cascade_remove_independent']);
            unset($config['cascade_remove_independent']);
        }

        if (array_key_exists('bidirectional_consistency', $config)) {
            $this->_usedProperties['bidirectionalConsistency'] = true;
            $this->bidirectionalConsistency = new \Symfony\Config\DoctrineDoctor\Analyzers\BidirectionalConsistencyConfig($config['bidirectional_consistency']);
            unset($config['bidirectional_consistency']);
        }

        if (array_key_exists('orphan_removal_no_cascade', $config)) {
            $this->_usedProperties['orphanRemovalNoCascade'] = true;
            $this->orphanRemovalNoCascade = new \Symfony\Config\DoctrineDoctor\Analyzers\OrphanRemovalNoCascadeConfig($config['orphan_removal_no_cascade']);
            unset($config['orphan_removal_no_cascade']);
        }

        if (array_key_exists('ondelete_mismatch', $config)) {
            $this->_usedProperties['ondeleteMismatch'] = true;
            $this->ondeleteMismatch = new \Symfony\Config\DoctrineDoctor\Analyzers\OndeleteMismatchConfig($config['ondelete_mismatch']);
            unset($config['ondelete_mismatch']);
        }

        if (array_key_exists('join_optimization', $config)) {
            $this->_usedProperties['joinOptimization'] = true;
            $this->joinOptimization = new \Symfony\Config\DoctrineDoctor\Analyzers\JoinOptimizationConfig($config['join_optimization']);
            unset($config['join_optimization']);
        }

        if (array_key_exists('doctrine_cache', $config)) {
            $this->_usedProperties['doctrineCache'] = true;
            $this->doctrineCache = new \Symfony\Config\DoctrineDoctor\Analyzers\DoctrineCacheConfig($config['doctrine_cache']);
            unset($config['doctrine_cache']);
        }

        if (array_key_exists('naming_convention', $config)) {
            $this->_usedProperties['namingConvention'] = true;
            $this->namingConvention = new \Symfony\Config\DoctrineDoctor\Analyzers\NamingConventionConfig($config['naming_convention']);
            unset($config['naming_convention']);
        }

        if (array_key_exists('missing_embeddable_opportunity', $config)) {
            $this->_usedProperties['missingEmbeddableOpportunity'] = true;
            $this->missingEmbeddableOpportunity = new \Symfony\Config\DoctrineDoctor\Analyzers\MissingEmbeddableOpportunityConfig($config['missing_embeddable_opportunity']);
            unset($config['missing_embeddable_opportunity']);
        }

        if (array_key_exists('blameable_trait', $config)) {
            $this->_usedProperties['blameableTrait'] = true;
            $this->blameableTrait = new \Symfony\Config\DoctrineDoctor\Analyzers\BlameableTraitConfig($config['blameable_trait']);
            unset($config['blameable_trait']);
        }
