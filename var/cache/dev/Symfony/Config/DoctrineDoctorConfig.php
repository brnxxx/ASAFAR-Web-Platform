...
class DoctrineDoctorConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists('analysis', $config)) {
            $this->_usedProperties['analysis'] = true;
            $this->analysis = new \Symfony\Config\DoctrineDoctor\AnalysisConfig($config['analysis']);
            unset($config['analysis']);
        }

        if (array_key_exists('analyzers', $config)) {
            $this->_usedProperties['analyzers'] = true;
            $this->analyzers = new \Symfony\Config\DoctrineDoctor\AnalyzersConfig($config['analyzers']);
            unset($config['analyzers']);
        }

        if (array_key_exists('profiler', $config)) {
            $this->_usedProperties['profiler'] = true;
            $this->profiler = new \Symfony\Config\DoctrineDoctor\ProfilerConfig($config['profiler']);
            unset($config['profiler']);
        }

        if (array_key_exists('debug', $config)) {
            $this->_usedProperties['debug'] = true;
            $this->debug = new \Symfony\Config\DoctrineDoctor\DebugConfig($config['debug']);
            unset($config['debug']);
        }
